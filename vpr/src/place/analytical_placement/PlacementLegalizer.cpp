...
public:
    ...
    ArchModelOld(const AtomNetlist &netlist) : atom_netlist(netlist) {
...
struct PlaceTile {
    ...
    //  - sub_tiles: the different sub_tiles on the tile that contain atoms
    ...
    // TODO: It make be a good idea to use a strong ID for the sub_tile.
    ...
    // Explicitly deleting the copy constructor to prevent bugs. This information
    // should only be modified by the TileGraph, everyone else is read-only.
    PlaceTile(const PlaceTile&) = delete;

    PlaceTile(int tile_x, int tile_y, int tile_layer_num, size_t index,
              const t_physical_tile_type &tile_type) : tile_index(index), physical_tile_type(tile_type), x(tile_x), y(tile_y), layer_num(tile_layer_num) {
    ...
    std::reference_wrapper<PlaceSubTile> get_sub_tile(size_t sub_tile_idx) {
        ...
        // TODO: Definately make this one debug.
...
class PlaceTileGraph {
    ...
    // TODO: It would be nice if this could be references to the tiles, but this
    //       should be fine.
...
public:
    ...
    PlaceTileId get_place_tile_id(int x, int y) {
    ...
    std::reference_wrapper<std::unordered_set<PlaceTileId>> get_overfilled_tiles() {
...
class SimpleArchModel {
    ...
    void add_atom_to_tile(size_t node_id, PlaceTileId tile_id) {
        // Simple model does not care for sub_tiles. Assumes there is only 1.
        size_t sub_tile_idx = 0;
        const PlaceTile& place_tile = tile_graph.get_place_tile(tile_id);
        const PlaceSubTile& sub_tile = place_tile.sub_tiles[sub_tile_idx];
        // Simple model is homogenous, assumes there is max capacity, and
        // everything is the same size.
        // If there is space in the sub_tile, add it to the sub_tile.
        if (sub_tile.atoms_in_site.size() < BIN_CAPACITY)
            tile_graph.add_atom_to_sub_tile(node_id, tile_id, sub_tile_idx);
        else
...
public:
    ...
    void move_atom_to_tile(size_t node_id, PlaceTileId target_tile_id) {
        ...
        // Move the atom
        // TODO: This is very similar to adding. Can it be made the same?
        const PlaceTile& target_tile = tile_graph.get_place_tile(target_tile_id);
        if (target_tile.sub_tiles[0].atoms_in_site.size() < BIN_CAPACITY)
    ...
    std::vector<size_t> get_overflowing_nodes(const PlaceTileId tile_id, size_t node_id, const PartialPlacement& p_placement) {
        // FIXME: This may get quite expensive if it is called often. Perhaps
        //        this can be stored?
        //    - there is a hidden O(n) copy.
        // For the simple, homogenous model, all nodes have the same type.
        // Other models may use this information.
        (void)node_id;
        (void)p_placement;
        // For the simple model, all nodes in the overflow sub tile are overflowing.
        std::vector<size_t> overflowing_nodes;
        const PlaceTile& place_tile = tile_graph.get_place_tile(tile_id);
        const auto& overflow_atoms = place_tile.overflow_sub_tile.atoms_in_site;
        return std::vector<size_t>(overflow_atoms.begin(), overflow_atoms.end());
    }
    ...
    float get_tile_overuse(const PlaceTileId tile_id, size_t node_id, const PartialPlacement& p_placement) const {
        ...
        const PlaceTile& place_tile = tile_graph.get_place_tile(tile_id);
        return place_tile.overflow_sub_tile.atoms_in_site.size();
    }
    ...
    float get_tile_underuse(const PlaceTileId tile_id, size_t node_id, const PartialPlacement& p_placement) const {
        ...
        return place_tile.sub_tiles[0].atoms_in_site.size() - BIN_CAPACITY;
    }

    void import_node_locations(PartialPlacement& p_placement) {
        ...
        for (size_t node_id = p_placement.num_moveable_nodes; node_id < p_placement.num_nodes; node_id++) {
            ...
            // Cannot allow any of the moveable nodes to go into the overflow
...
void FlowBasedLegalizer::legalize(PartialPlacement &p_placement) {
    ...
    ArchModelOld arch_model(p_placement.atom_netlist);
