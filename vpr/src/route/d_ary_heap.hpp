// Note for VPR developers: This D-ary heap implementation should be removed
// from VPR codebase eventually, as there are no clues that this implementation
// has outstanding performance compared to a VPR-customized heap (e.g., the VPR
// Binary Heap).
...
void make_dary_heap(It begin, It end) {
    make_dary_heap<D>(begin, end, std::greater<>{});
}
...
bool is_dary_heap(It begin, It end) {
    return is_dary_heap<D>(begin, end, std::greater<>{});
}
...
void push_dary_heap(It begin, It end) {
    return push_dary_heap<D>(begin, end, std::greater<>{});
}
...
void pop_dary_heap(It begin, It end) {
    return pop_dary_heap<D>(begin, end, std::greater<>{});
}
